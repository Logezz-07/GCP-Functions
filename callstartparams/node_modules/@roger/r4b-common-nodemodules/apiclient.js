...
const TOKEN_REFRESH_THRESHOLD_MIN = 55;

const TOKEN_API_URL = "https://login.microsoftonline.com/0ab4cbbf-4bc7-4826-b52c-a14fed5286b9/oauth2/v2.0/token";


async function updateSecret(name, value) {
    const SECRET_NAME = `projects/roger-470808/secrets/${name}`;
...
async function getValidToken({ sessionId, tag, secretHeader }) {
    ...
    if (generateNew) {
        ...
        await updateSecret("R4B-Auth-Token", newToken);
        await updateSecret("R4B-Auth-Token-ExpiryTime", newExpiry);
